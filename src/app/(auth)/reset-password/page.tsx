...
function ResetForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) setError(data.message ?? data.error ?? 'Une erreur est survenue.')
      else setDone(true)
  ...
  if (!token) {
    return (
      <div className="text-center space-y-4">
        ...
        <Link href="/forgot-password" className="text-sm text-brand-600 hover:underline">Demander un nouveau lien</Link>
      </div>
    )
  }
  ...
  if (done) {
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-sm text-gray-500">Connectez-vous avec votre nouveau mot de passe.</p>
        <Link href="/login" className="inline-block w-full bg-brand-700 text-white font-semibold text-sm py-2.5 rounded-md text-center">Se connecter</Link>
      </div>
    )
  }
